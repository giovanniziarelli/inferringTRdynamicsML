...
folder = '/home/giovanni/Desktop/LDNets/neurons_' + str(neurons) + '_' + str(T_fin) + '_synthetic_lay' + str(layers) + '_err_1e_2_Testg_trial_'+str(trial)+'_provaestim_IG/'
...
def IC_fun(num_samples):
    ...
    IC[:,1] = np.minimum(np.random.uniform(0,   0.04, num_samples), 1 - IC[:,0])#1 - IC[:,0] - IC[:,2]
    IC[:,2] = 1 - IC[:,0] - IC[:,1] #np.minimum(np.random.uniform(0,0.1,num_samples), 1 - IC[:,0])
    return IC

if various_IC > 0:
    IC = IC_fun(n_size)
    IC_testg = IC_fun(n_size_testg)
...
for i in range(t_num.shape[1]-1):
    ...
    beta_real_testg[:, i+1, :] = beta_real_testg[:, i, :] + dt_num * beta_evolution(beta_real_testg[:,i,:], testing_var_numpy[:,i][:,None], delta_test[:,None], T_mean[:,None])
...
# dynamics network
# Initial conditions
InitialValue_train = 0.00001 * np.ones((dataset_train['num_samples'], 2))
InitialValue_testg = 0.00001 * np.ones((dataset_testg['num_samples'], 2))
...
def evolve_model(dataset, lat_state, IC_d):
    IC_E = tf.reshape(IC[:,0], [IC_d.shape[0],1])
    IC_I = tf.reshape(IC[:,1], [IC_d.shape[0],1])
    IC_S = tf.reshape(1 - IC_I - IC_E, [IC_d.shape[0], 1])
    IC_all = tf.concat([IC_S, IC_E, IC_I], 1)
    #tf.print(IC_all)
    state = IC_all
...
#%% Loss weights
nu = 1.1e-4#1e-1#1e-4#5e-2#5e-2#5.1e0#1e-1#i1e0 good 1e0#1e-7#1e-2#-4#1e0#1e2
nu_h1 = 0#1e-3 
nu_r1  = 1e-1#1e-7#1e-6#1e-2#5e-3#1e-5
nu_p = 1e0#1e4#1e0#1e-1#6
nu_p2 = 1e0#1e4#5e0#1e-1#1e-3#1e-3#6
alpha_reg = 0#1e-1#1e-3#1e-6#1e-1#1e-2#1e#1e-5#1e-2#1e-2#1e-1# 1e-8#1e-8#1e1#1e-2#1e-3#5e-5#5e-5#5e-6#4.7e-3
alpha_reg_IC = 1e10#1e-4#1e-3
#%% Training
...
def loss_train():
    ...
    l = nu *loss_exp_beta(dataset_train, beta, IC_train)\
            ...
            + alpha_reg_IC * tf.reduce_mean(tf.minimum(tf.pow(100 * IC_train, 5), tf.zeros_like(IC_train)))\
            + alpha_reg_IC * tf.reduce_mean(tf.maximum(tf.pow(100 * (IC_train-0.05), 5), tf.zeros_like(IC_train)))
    return l 
...
num_epochs_BFGS_train = 10000#10000
...
if num_latent_params>0:
    ...
    #%% Constants
    nu_r2 = 2.4e1#1e-2#5e-3#1e-5
    nu_loss_testg = 5e-2#1e-1
    nu_IC = 0#1e-3
    nu_p_testg = 0#1e2
    nu_p2_testg = 1e2
...
os.system('cp /home/giovanni/Desktop/LDNets/src/TestCase_epi_external_dataset_synthetic_scenario_test.py ' + folder)
