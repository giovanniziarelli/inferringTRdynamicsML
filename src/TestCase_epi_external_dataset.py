...
dt = 0.2
t_max = 12
b_ref = 0.1#0.5#0.025#0.7#0.77#0.1672#0.92851
tau = 0
layers = 3
err = 1e-1#1e-1
...
folder = '/home/giovanni/Desktop/LDNets/trial_T_obs_'+ str(T_obs)+'_neurons_' + str(neurons) + '_' + str(T_fin) + '_ITALY_lay' + str(layers) + '/'
...
def smoothing(data):
    smoothed_data = savgol_filter(data, 197, 2) #savgol_filter(data, 31, 2)
...
dataset_train = {
        ...
        'inp_signals' : training_var_numpy[:num_train,:,None], # [num_samples x num_times x num_signals]
...
#%% Loss weights
nu = 5e0#1.1e-2#1e-1#3e-1#1.8e-1#1e-1#i1e0 good 1e0#1e-7#1e-2#-4#1e0#1e2
nu_h1 = 0#1e-1 
nu_r1  = 2.5e-3#5e-3#1.7e-3#e-5
nu_p = 5e1#5e-4#5e-2#5e-2#1e-2#6
nu_p2 = 5e-2#5e0#5e-2#1e-3#6
nu_p3 = 5e-1#5e0#1e0#1e-3#6
nu_s_inf = 1e-2#5e-2#1e-2
alpha_reg = 5e-3#1e-3#1e#1e-5#1e-2#1e-2#1e-1# 1e-8#1e-8#1e1#1e-2#1e-3#5e-5#5e-5#5e-6#4.7e-3
...
def loss_train():
    ...
    l = nu *loss_exp_beta(dataset_train, beta, IC_train)\
            ...
            + nu_p * tf.reduce_mean(tf.maximum(tf.pow(10 * (beta - 2), 5), tf.zeros_like(beta)))\
...
def loss_train_abs():
    ...
    l = nu *loss_exp_beta_abs(dataset_train, beta, IC_train)\
            + alpha_reg * weights_reg(NNdyn)\
            + nu_r1 * tf.reduce_mean(tf.square(lat_param_train-1))\
            + nu_p * tf.reduce_mean(tf.maximum(tf.pow((beta - 2), 5), tf.zeros_like(beta)))\
            + nu_p2 * tf.reduce_mean(tf.square((initial_lat_state_train - beta[:,1,:]) / dt_num))\
            + nu_s_inf * loss_s_inf(dataset_train, beta, IC_train)\
            + nu_p3 * tf.reduce_mean(tf.reduce_max(tf.square((beta_coarse[:,1:] - beta_coarse[:,:-1])/(dt_num*2*7 )), axis = 1))
...
def val_train_2():
    beta = evolve_dynamics(dataset_train, initial_lat_state_train, lat_param_train)
    l = tf.reduce_mean(tf.maximum(tf.pow((beta - 0.9), 5), tf.zeros_like(beta)))
    return l 
...
if num_latent_params>0:
    ...
    #%% Constants
    nu_r2 = 6.5e-2#1e0#3.3ee0#1.85e-1#5e-3#1e-5
    nu_loss_testg = 1e-1#5e-2#1.7e-2#7e-2#1e-1
    nu_b0 = 7e-2#1e-2#5.9e-4
...
########        + nu_p2 * tf.reduce_mean(tf.square((initial_lat_state_testg - evolve_dynamics(dataset_testg, initial_lat_state_testg, lat_param_testg)[:,1,:]) / dt_num))
    ...
    num_epochs_Adam_testg = 1000
    num_epochs_BFGS_testg = 5000
    
    print('training (Adam)...')
    opt_testg.optimize_keras(num_epochs_Adam_testg, tf.keras.optimizers.Adam(learning_rate=1e-5))
    print('training (BFGS)...')
...
if save_testg:
   ...
   #####plt.show()
    ...
    print(delta_train_r)
    print(delta_testg_r)
    tf.print(initial_lat_state_train)
    tf.print(initial_lat_state_testg)
...
os.system('cp /home/giovanni/Desktop/LDNets/src/TestCase_epi_external_dataset.py ' + folder)
