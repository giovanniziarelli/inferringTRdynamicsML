...
folder = '/home/giovanni/Desktop/LDNets/neurons_' + str(neurons) + '_' + str(T_fin) + '_days_random_20_lay' + str(layers) +'_noised_0_1_10/'
...
def evolve_model(dataset, lat_state):
    state = dataset['initial_state']
...
def LDNet(dataset, initial_lat_state, lat_param):
    lat_states = evolve_dynamics(dataset,initial_lat_state, lat_param)
    return evolve_model(dataset, lat_states)
...
def loss_exp(dataset, initial_lat_state, lat_param):
    state = LDNet(dataset, initial_lat_state, lat_param)
...
alpha_reg = 1e-6# 1e-8#1e-8#1e1#1e-2#1e-3#5e-5#5e-5#5e-6#4.7e-3
...
loss_train = lambda: loss(dataset_train, initial_lat_state_train, lat_param_train)  + alpha_reg * weights_reg(NNdyn)+ nu_r1 * tf.reduce_mean(tf.square(lat_param_train))# + alpha_reg * weights_reg(NNdyn) 
...
# TO DO: regularization on lat_param_testg
    ...
    loss_testg = lambda: nu_loss_testg * loss(dataset_testg, initial_lat_state_testg, lat_param_testg) + nu_r2 * tf.reduce_mean(tf.transpose((lat_param_testg - tf.reduce_mean(lat_param_train))) * C_post * (lat_param_testg - tf.reduce_mean(lat_param_train)))
    ...
    state_predict = LDNet(dataset_testg_full, initial_lat_state_testg, lat_param_testg)
    print('Cpost: ', C_post)
...
if plot_train:
    ...
    state_r = LDNet(dataset_train, initial_lat_state_train, lat_param_train).numpy()
