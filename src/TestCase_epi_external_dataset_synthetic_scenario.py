...
folder = '/home/giovanni/Desktop/LDNets/neurons_' + str(neurons) + '_' + str(T_fin) + '_synthetic_lay' + str(layers) + '_TRAIN_err_4e_2_trial' +str(trial) +  '_2/'
#folder = '/home/giovanni/Desktop/LDNets/target_err_3e_2_Train/'
...
dataset_path = '/home/giovanni/Desktop/LDNets/datasets/temperature_100.pkl'#temperature_not_modified_new_b_ref_40.pkl'#temperature_n.pkl'
dataset_path = '/home/giovanni/Desktop/LDNets/datasets/temperature_50.pkl'#temperature_not_modified_new_b_ref_40.pkl'#temperature_n.pkl'
with open(dataset_path, 'rb') as file:
    dataset_extern, val_extern, test_extern, beta0_train, beta0_val, beta0_test, T_mean, amp_T, phase_T, f_T = pickle.load(file)

betas, deltas = shf.compute_beta_delta([dataset_extern, val_extern, test_extern], \
        [beta0_train, beta0_val, beta0_test],\
        t_max, tau, b_ref, T_mean)
...
for i in range(t_num.shape[1]-1):
    beta_real[:, i+1, :] =beta_real[:, i, :] + dt_num * beta_evolution(beta_real[:,i,:], training_var_numpy[:,i][:,None], delta_train[:,None], T_mean[:,None])
...
beta_real_mean = np.zeros((n_size, t_num.shape[1]))
beta_real_mean = np.array([np.mean(beta_real[k,:]) for k in range(beta_real.shape[0])])#np.mean(beta_real.squeeze(), axis = 1)
print(beta_real_mean.shape)
state_mod_const = state_mod.copy()

for i in range(t_num.shape[1]-1):
    state_mod_const[:,i+1,:] = state_mod_const[:,i,:] + dt_num * epiModel_rhs(state_mod_const[:,i,:], beta_real_mean[:,None])
print('beta qui')
...
beta_real_mean = beta_real_mean[shuffle_indexes]
state_mod_const = state_mod_const[shuffle_indexes]
...
if plot_temps:
    ...
    for k in range(training_var_numpy.shape[0]-40):
        #ax[0].plot(training_var_numpy[k,:])

        #ax[1].plot(cases[k,:])
        
        ax[0].plot(beta_real[k,:])
        ax[1].plot(I_interp[k,:])
    
        ax[2].plot([beta_real_mean[k]] * t_num.shape[1])
        ax[3].plot(state_mod_const[k,:, 2])
...
InitialValue_testg[:,0] = IC[:,2]# * np.ones_like(cases[:,0])#cases[:,0]#10 / 50000000
...
def loss_exp_beta(dataset, lat_states, IC):
    ...
    MSE = tf.reduce_mean(tf.square(1*(S[:,:-1] + E[:,:-1] - S[:,1:] - E[:,1:]) - 1* dataset['target_cases']) / tf.square(1* dataset['target_cases']))#(S[:,:-1] + E[:,:-1] - S[:,1:] - E[:,1:])+ 1e-6 )) 
...
#%% Loss weights
nu = 1.1e-2#1e-1#1e-4#5e-2#5e-2#5.1e0#1e-1#i1e0 good 1e0#1e-7#1e-2#-4#1e0#1e2
nu_h1 = 0#1e-3 
nu_r1  = 1e-5#1e-6#1e-2#5e-3#1e-5
nu_p = 1e2#1e4#1e0#1e-1#6
nu_p2 = 1e2#1e4#5e0#1e-1#1e-3#1e-3#6
alpha_reg = 0#1e-1#1e-3#1e-6#1e-1#1e-2#1e#1e-5#1e-2#1e-2#1e-1# 1e-8#1e-8#1e1#1e-2#1e-3#5e-5#5e-5#5e-6#4.7e-3
...
num_epochs_Adam_train = 2000# good
...
if num_latent_params>0:
    ...
    #%% Constants
    nu_r2 = 3e1#2.4e1#1e-2#5e-3#1e-5
    nu_loss_testg = 1e-1#5e-2#1e-1
    nu_IC = 0#1e-3
    nu_p_testg = 0#1e2
    nu_p2_testg = 1e2
...
if save_testg:
    ...
    axs_delta.plot(delta_train_r, delta_train_r, '-')
    ...
    np.savetxt(folder_testg + 'S_real_testg.txt', state_mod[:,:,0])
    np.savetxt(folder_testg + 'E_real_testg.txt', state_mod[:,:,1])
    np.savetxt(folder_testg + 'I_real_testg.txt', state_mod[:,:,2])
    np.savetxt(folder_testg + 'cases_real_testg.txt', cases)
    np.savetxt(folder_testg + 'beta_real_testg.txt', beta_real.squeeze())
os.system('cp /home/giovanni/Desktop/LDNets/src/TestCase_epi_external_dataset_synthetic_scenario.py ' + folder)
