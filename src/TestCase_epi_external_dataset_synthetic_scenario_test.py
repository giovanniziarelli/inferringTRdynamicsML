...
folder = '/u/archive/dott/ziarelli/LDNets_cluster/neurons_' + str(neurons) + '_' + str(T_fin) + '_synthetic_lay' + str(layers) + '_err_'+err_name+'_size_'+size[trial-1] + '_trialsim_' + str(trial_sim) +'/'
...
dataset_path = '/u/archive/dott/ziarelli/LDNets_cluster/datasets/temperature_'+size[trial-1]+'.pkl'
test_path    = '/u/archive/dott/ziarelli/LDNets_cluster/datasets/temperature_50.pkl'
...
def loss_exp_beta(dataset, lat_states, IC):
    ...
    MSE = tf.reduce_mean(tf.square(1000*(S[:,:-1] + E[:,:-1] - S[:,1:] - E[:,1:]) - 1000* dataset['target_cases']) / tf.square(1000* dataset['target_cases']))#(S[:,:-1] + E[:,:-1] - S[:,1:] - E[:,1:])+ 1e-6 )) 
...
def loss_exp_beta_abs(dataset, lat_states, IC):
    ...
    MSE = tf.reduce_mean(tf.square(1000*(S[:,:-1] + E[:,:-1] - S[:,1:] - E[:,1:]) - 1000* dataset['target_cases']))#(S[:,:-1] + E[:,:-1] - S[:,1:] - E[:,1:])+ 1e-6 )) 
...
nu_h1 = 0#1e-3 
nu_r1  = 1e-3#1e-6#1e-2#5e-3#1e-5
nu_p = 1e1#1e4#1e0#1e-1#6
nu_p2 = 1e1#1e4#5e0#1e-1#1e-3#1e-3#6
alpha_reg = 5e-6#1e-1#1e-3#1e-6#1e-1#1e-2#1e#1e-5#1e-2#1e-2#1e-1# 1e-8#1e-8#1e1#1e-2#1e-3#5e-5#5e-5#5e-6#4.7e-3
...
num_epochs_Adam_train = 500# good
...
if num_latent_params>0:
    ...
    #%% Constants
    nu_r2 = 1e0#3e1#2.4e1#1e-2#5e-3#1e-5
    nu_r3 = 5e-1#3e1#2.4e1#1e-2#5e-3#1e-5
    nu_loss_testg = 5e-1#5e-2#1e-1
    nu_IC = 0#1e-3
    nu_p_testg = 0#1e2
    nu_p2_testg = 1e0#1e2
...
os.system('cp /u/archive/dott/ziarelli/LDNets_cluster/src/TestCase_epi_external_dataset_synthetic_scenario_test.py ' + folder)
#time.sleep(35000)
