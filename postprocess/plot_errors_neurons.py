...
output_folder = '/home/giovanni/Desktop/LDNets/pprocess_images/errs_neurons/'
if not os.path.exists(output_folder):
    os.mkdir(output_folder)
for j in range(len(neurons)):

    for i in range(n_tests):
        folder_name = '/home/giovanni/Desktop/LDNets/neurons_'+ str(neurons[j])+'_364_synthetic_lay2_err_0_size_50_trialsim_' +str(i+1) + '/train/'
        folder_name_testg = '/home/giovanni/Desktop/LDNets/neurons_'+ str(neurons[j])+'_364_synthetic_lay2_err_0_size_50_trialsim_' +str(i+1) + '/testg/'
...
plt.plot(neurons, median_err_train, '--o', color = color_median, label='Median')
plt.fill_between(neurons, qmin_err_train, qmax_err_train, color = color_area, alpha=0.5, label=label_quantiles, linewidth=0)
plt.legend()
plt.xlabel(r'size')
...
plt.semilogy(neurons, median_err_testg, 'o', color = color_median, markersize=10)
plt.vlines(neurons, ymin=qmin_err_testg, ymax=qmax_err_testg, colors=color_area, linestyles='dashed', linewidth=2)
plt.hlines(y=qmin_err_testg, xmin=[xi-0.05 for xi in neurons], xmax=[xi+0.05 for xi in neurons], colors=color_area, linewidth = 2)
plt.hlines(y=qmax_err_testg, xmin=[xi-0.05 for xi in neurons], xmax=[xi+0.05 for xi in neurons], colors=color_area, linewidth = 2)
#plt.fill_between(neurons, qmin_err_testg, qmax_err_testg, color = color_area, alpha=0.5, label=label_quantiles, linewidth=0)
plt.legend()
plt.xlabel(r'size')
...
plt.plot(neurons, median_err_train_testg, '--o', color = color_median)
plt.fill_between(neurons, qmin_err_train_testg, qmax_err_train_testg, color = color_area, alpha=0.5, label=label_quantiles, linewidth=0)
plt.legend()
plt.xlabel(r'size')
