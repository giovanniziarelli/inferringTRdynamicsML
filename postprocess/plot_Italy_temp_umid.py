...
output_folder = '' # ex '/home/giovanni/Desktop/LDNets/pprocess_images/ita_temp_umid/' 
...
for i in range(n_tests):
    folder_name_train = '' # ex '/home/giovanni/Desktop/LDNets/try_neurons_4_196_lay2_umidity_Tobs_50_trialsim'+str(i+1)+'/train/'#niente#_rd_2#_rd_4
    folder_name_testg = '' # ex '/home/giovanni/Desktop/LDNets/try_neurons_4_196_lay2_umidity_Tobs_50_trialsim'+str(i+1)+'/testg/'#niente#_rd_2#_rd_4 
    if i == 0:
...
# from Trentini A/H1N1, A/H3N2, B we extract athe average reproduction effective number
...
for i in range(9):
    plt.scatter(reff_perc[order_season_train[i]], median_delta_train[i],s=100,c=col[order_season_train[i]], label=seasons[order_season_train[i]])
plt.plot(ev_x, evals)
plt.errorbar(reff_perc[order_season_testg], median_delta_testg, errors_testg, fmt = 'o',color='k', markersize=0.0001, alpha=0.2 )
plt.scatter(reff_perc[order_season_testg], np.array(median_delta_testg).reshape((1,)), s=100,c='white',edgecolors = 'k', marker='*', label=seasons[order_season_testg[0]])
