...
folder = '/home/giovanni/Desktop/LDNets/save_neurons_' + str(neurons) + '_' + str(T_fin) + '_lay' + str(layers) + '_umidity_Tobs_' + str(T_obs)+ '_trialsim'+ str(trial_sim)+ '/'
...
#%% Loss weights
nu = 1e-1#1.5e-2#3e-1#1.8e-1#1e-1#i1e0 good 1e0#1e-7#1e-2#-4#1e0#1e2
nu_h1 = 0#1e-1 
...
if num_latent_params>0:
    ...
    #%% Constants
    nu_r2 = 6.2e0#6.2e0#1e-3#6.5e-3#5.6e0#3.8e0#5e-1#1.85e-1#5e-3#1e-5#quattro ordini in meno
...
########        + nu_p2 * tf.reduce_mean(tf.square((initial_lat_state_testg - evolve_dynamics(dataset_testg, initial_lat_state_testg, lat_param_testg)[:,1,:]) / dt_num))
    ...
    num_epochs_Adam_testg = 10000
    num_epochs_BFGS_testg = 3000
    
    nu_loss_testg = 7e-2#1e-2#8e-2#5e-3#1e-3#1.7e-1#prima era 1.7e-2
    print('training (Adam)...')
    opt_testg.optimize_keras(num_epochs_Adam_testg, tf.keras.optimizers.Adam(learning_rate=1e-4))# tf.keras.optimizers.Adam(learning_rate=5e-3))
    print('training (BFGS)...')
