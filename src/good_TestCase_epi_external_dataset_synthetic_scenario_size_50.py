...
err_vec = [0, 1e-3, 5e-3, 1e-2, 5e-2, 1e-1, 5e-1]#1e-1#1e-1#1e-1
err_vec_name = ['0', '1e-3', '5e-3', '1e-2', '5e-2', '1e-1', '5e-1']
err_name = err_vec_name[trial_err-1]
err = err_vec[trial_err-1]

size = ['25','50', '75','100','150', '500']#'200', '300', '400', '500', '750', '1000']
size_num = [25, 50, 75, 100, 150, 500]#200, 300, 400, 500, 750, 1000]
...
folder = 'neurons_' + str(neurons) + '_' + str(T_fin) + '_synthetic_lay' + str(trial_lay+1) + '_err_'+err_name+'_size_'+size[trial-1] + '_trialsim_' + str(trial_sim) +'_/'
#folder = '/home/giovanni/Desktop/LDNets/target_err_3e_2_Testg/'
ssize = size_num[trial-1]
...
print('num_samples')
print(dataset_train['num_samples'])
...
nu_h1 = 0#1e-3 
nu_r1  = 5e-3#1e-6#1e-2#5e-3#1e-5
nu_p = 1e-3#1e1#1e4#1e0#1e-1#6
nu_p2 = 1e-3#1e1#1e4#5e0#1e-1#1e-3#1e-3#6
alpha_reg = 1e-8#1e-1#1e-3#1e-6#1e-1#1e-2#1e#1e-5#1e-2#1e-2#1e-1# 1e-8#1e-8#1e1#1e-2#1e-3#5e-5#5e-5#5e-6#4.7e-3
...
num_epochs_Adam_train =500# 200# good
num_epochs_BFGS_train = 1000#200

init_adam_time = time.time()
print('training (Adam)...')
opt_train.optimize_keras(num_epochs_Adam_train, tf.keras.optimizers.Adam(learning_rate=5e-3 *25 / ssize))#size_num[trial-1]))#1e-3))#1e-2 good
...
num_epochs_Adam_train = 500#200# good
num_epochs_BFGS_train = 1000#500

init_adam_time = time.time()
print('training (Adam)...')
opt_train.optimize_keras(num_epochs_Adam_train, tf.keras.optimizers.Adam(learning_rate=1e-6 * 20 /100 *25/ssize))#size_num[trial-1]))#1e-3))#1e-2 good
end_adam_time = time.time()
